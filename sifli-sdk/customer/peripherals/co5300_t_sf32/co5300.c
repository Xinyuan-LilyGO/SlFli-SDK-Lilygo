 ...
 */
#define REG_ORIENTATION_PORTRAIT                                               \
    (0x00) /* Portrait orientation choice of LCD screen  */
#define REG_ORIENTATION_LANDSCAPE                                              \
    (0x01) /* Landscape orientation choice of LCD screen */
#define REG_ORIENTATION_LANDSCAPE_ROT180                                       \
    (0x02) /* Landscape rotated 180 orientation choice of LCD screen */
...
static LCDC_InitTypeDef lcdc_int_cfg_qadspi = {
    ...
    .freq = LCD_MAX_CLK_FREQ, // CO5300 RGB565 only support 50000000,  RGB888
                              // support 60000000
#else
    .freq = 50000000, // CO5300 RGB565 only support 50000000,  RGB888 support
                      // 60000000
#endif
...
static void LCD_Init(LCDC_HandleTypeDef *hlcdc)
{
    ...
    // rt_kprintf("Using QAD SPI interface for LCD\n");
                                    ...
                                    uint16_t Ypos1)
{
    ...
    HAL_LCDC_SendLayerData2Reg_IT(hlcdc, ((0x32 << 24) | (REG_WRITE_RAM << 8)),
                                  4);
}
                             ...
                             uint8_t ReadSize)
{
    ...
    ret = HAL_LCDC_ReadU32Reg(hlcdc, ((0x03 << 24) | (RegValue << 8)),
                              (uint8_t *)&rd_data, ReadSize);
    // if (ret != HAL_OK)
    // {
    //     LOG_E("LCD Read Reg 0x%04x failed, ret=%d", RegValue, ret);
    // }
    // else
    // {
    //     LOG_E("LCD Read Reg success");
    // }
                              ...
                              uint16_t Ypos)
{
    ...
    case LCDC_PIXEL_FORMAT_RGB565:
        ret_v = (uint32_t)(((r << 11) & 0xF800) | ((g << 5) & 0x7E0) |
                           ((b >> 3) & 0X1F));
        break;
    ...
    */
    case LCDC_PIXEL_FORMAT_RGB888:
        ret_v = (uint32_t)(((r << 16) & 0xFF0000) | ((g << 8) & 0xFF00) |
                           ((b) & 0XFF));
        break;
...
static const LCD_DrvOpsDef CO5300_drv = {
    LCD_Init,         LCD_ReadID,        LCD_DisplayOn,
    LCD_DisplayOff,

    LCD_SetRegion,    LCD_WritePixel,    LCD_WriteMultiplePixels,

    LCD_ReadPixel,

    LCD_SetColorMode, LCD_SetBrightness, LCD_IdleModeOn,
    LCD_IdleModeOff,

};

LCD_DRIVER_EXPORT(co5300, LCD_ID, &lcdc_int_cfg, &CO5300_drv, LCD_PIXEL_WIDTH,
                  LCD_PIXEL_HEIGHT, 2);

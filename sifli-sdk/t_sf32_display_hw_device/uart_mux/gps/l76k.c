...
static rt_size_t gps_nmea_len = 0;

static void gps_nmea_debug_input(uint8_t ch)
{
    if (ch == '$')
    {
        gps_nmea_len = 0;
    ...
    if (gps_nmea_len < GPS_NMEA_LINE_MAX - 1)
    {
        gps_nmea_line[gps_nmea_len++] = (char)ch;
    }
    else
    {
        gps_nmea_len = 0;
...
int l76k_set_mode(L76K_Mode mode)
{
    ...
    switch (mode)
    {
    case L76K_MODE_STANDBY:
        cmd = "$PMTK161,0*28\r\n";
        len = strlen(cmd);
        break;
    case L76K_MODE_NORMAL:
        cmd = "$PMTK010,1*2E\r\n";
        len = strlen(cmd);
        break;
    case L76K_MODE_SLEEP:
        cmd = "$PMTK161,1*29\r\n";
        len = strlen(cmd);
        break;
    case L76K_MODE_BACKUP:
        cmd = "$PMTK161,2*2A\r\n";
        len = strlen(cmd);
        break;
    default:
...
int l76k_send_cmd(const char *cmd, size_t len)
{
    if (uart_mux_get_current_device() != UART_MUX_DEVICE_GPS)
    {
        uart_mux_switch_to(UART_MUX_DEVICE_GPS, L76K_DEFAULT_BAUDRATES);
    }
...
#ifdef RT_USING_PM
static int l76k_pm_suspend(const struct rt_device *device, uint8_t mode)
{
    l76k_set_mode(L76K_MODE_SLEEP); // GPS 模块进入低功耗
    uart_mux_register_rx_callback(UART_MUX_DEVICE_GPS, RT_NULL);
    l76k_deinit();
    return 0;
}

static void l76k_pm_resume(const struct rt_device *device, uint8_t mode)
{
    l76k_init("uart2", 115200); // 完整重初始化
}
static const struct rt_device_pm_ops l76k_pm_op = {
    .suspend = l76k_pm_suspend,
    .resume = l76k_pm_resume,
};
#endif
...
int l76k_init(const char *uart_name, uint32_t baudrate)
{
    ...
    if (uart_mux_switch_to(UART_MUX_DEVICE_GPS, L76K_DEFAULT_BAUDRATES) !=
        RT_EOK)
    {
